               ...
               <div class="content">
				<?php
					 if(isset($read) && $read != NULL){
						...
						/*
						if($read['news_images'] != NULL){
							echo "<img src='".base_url()."uploads/news/$read[news_images]' width='140' class='news_dt' />";
						}
						*/
						echo "<h3 class='news_detail_h3'>".$read['news_title']."</h3>";
						echo "<div class='news_detail_date'><i>".$read['news_date']." - ".$read['news_author']."</i></div>";
						echo "<div class='info'>";
						echo $read['news_info'];
						echo "</div>";
						echo "<div>";
				...
				<div class="liqu">
				<h4 style="padding:10px 0px;color:#00F;font:15px Tahoma, Geneva, sans-serif;font-weight:bold;">Tin liên quan</h4>
				<?php
					if(isset($liqu) && $liqu != NULL){
						echo "<ul>";
						foreach($liqu as $lq){
							echo "<li><a href='".base_url()."vn/news/index/$lq[news_id]'>$lq[news_title]</a></li>";
						}
						echo "</ul>";
					}else{
						echo "Không có tin liên quan";
					}
				?>
        </div>
            </div>
